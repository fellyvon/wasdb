...
import com.waspring.wasdb.util.StringUtils;
...
public class RowComparator implements Comparator {
	...
	public RowComparator(int i, boolean flag) {
		...
		if (flag)
			...
			byte0 = 1;
		compareColumns = (new CompareItem[] { new CompareItem(i, byte0) });
	}
	...
	public int compare(Object obj, Object obj1) throws ClassCastException {
		...
		for (int i = 0; i < compareColumns.length; i++) {
			...
			try {
				...
				if (obj3 == null) {
					...
					k = StringUtils.compareString(obj2.toString(),
							obj3.toString(), "GBK");
				else
