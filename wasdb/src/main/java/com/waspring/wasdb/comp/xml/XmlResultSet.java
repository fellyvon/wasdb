...
import com.waspring.wasdb.comp.BlobField;
import com.waspring.wasdb.comp.CachedRow;
import com.waspring.wasdb.comp.CompareItem;
import com.waspring.wasdb.comp.DBSetHandlerBase;
import com.waspring.wasdb.comp.DataPackageHandlerBase;
import com.waspring.wasdb.comp.ResultHandlerBase;
import com.waspring.wasdb.comp.RowComparator;
import com.waspring.wasdb.util.ByteArrayUtils;
import com.waspring.wasdb.util.PropertyFieldUtils;

/**
 * JDBC结果集类自定义实现，将结果集处理为XML 此类为wasdb项目的核心，但高版本的JdBC接口未完全实现。
 * 
 * @author felly
 * 
 */
		...
		try {
			...
			if (class1 == null)
				throw new RuntimeException((new StringBuilder())
						.append("Unsupport xmlDataType :").append(s).toString());
	...
	public String getString(int i) throws SQLException {
		...
		if (obj instanceof SerialBlob) {
			...
			if ((int) serialblob.length() > 0)
				return new String(serialblob.getBytes(1L,
						(int) serialblob.length()));
			else {
				return "";
			}
		} else {
		...
		if (columnValues != null) {
			...
			if (i < 1 || i > j)
				throw new SQLException((new StringBuilder())
						.append("Index Error").append(i).toString());
			if (i > columnValues.length) {
 ...
 */
	...
	public CachedRow[] getRows() {
		...
		for (int j = 0; j < i; j++)
			acachedrow[j] = new CachedRow(this,
 ...
 */
  ...
  */
	public void sortBy(int i, boolean flag) throws SQLException {
		...
		if (i < 1 || i > j) {
			throw new SQLException((new StringBuilder())
					.append("column not Finded!no=").append(i).toString());
		} else {
	...
	/**
	 * 类似SQL语句的union操作，不过这里将处理放到内存中进行。
	 * @param resultset
	 * @throws SQLException
	 */
		...
		try {
			...
			do {
				...
				XmlResultHandlerBase xmlresulthandlerbase = (XmlResultHandlerBase) class1
						.newInstance();
				if (ByteArrayUtils.indexOf(abyte0,
						xmlresulthandlerbase.getXmlCharacter()) != -1) {
	...
	protected XmlResultSetMetaData resultSetMetaData;//XML形式的元数据集合
	protected int rowCount;///数据集行数
	protected transient InputStream xmlStream;/////输入数据流
	...
	private ArrayList rowList;//存储行数据
	private Object columnValues[];///存储列值
	private String columnNames[];//存储列名
