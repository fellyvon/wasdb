 ...
 *
 */
public class BlobField implements Serializable {
	...
	public BlobField(Blob blob) throws SQLException {
		length = 0;
		buffer = null;
		length = (int) blob.length();
		buffer = new byte[length];
		BufferedInputStream bufferedinputstream = new BufferedInputStream(blob
				.getBinaryStream());
		try {
			int i = 0;
			int j = 0;
			do {
				i = bufferedinputstream.read(buffer, j,
						(int) ((long) length - (long) j));
				j += i;
			} while (i > 0 && j < length);
		} catch (IOException ioexception) {
			throw new SQLException((new StringBuilder()).append("SerialBlob: ")
