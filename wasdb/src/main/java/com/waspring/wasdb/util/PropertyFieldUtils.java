...
import com.waspring.wasdb.comp.PropertyField;
import com.waspring.wasdb.comp.PropertyFieldDescriptor;
...
public class PropertyFieldUtils {
	...
	private PropertyField getPropertyFieldDefine(String s) throws Exception {
		PropertyFieldDescriptor propertyfielddescriptor = (PropertyFieldDescriptor) classprops
				.get(s);
		...
		if (propertyfielddescriptor == null) {
			...
			if (as.length > 1) {
				...
				PropertyFieldUtils propertyfieldutils = new PropertyFieldUtils(
						obj);
				propertyfielddescriptor = (PropertyFieldDescriptor) propertyfieldutils.classprops
						.get(as[as.length - 1]);
			}
		}
		if (propertyfielddescriptor == null) {
	...
	public void setPropertyField(String s, Object obj) throws Exception {
	...
	public PropertyFieldDescriptor[] getFieldDescriptor() {
		PropertyFieldDescriptor apropertyfielddescriptor[] = new PropertyFieldDescriptor[classprops
				.size()];
		apropertyfielddescriptor = (PropertyFieldDescriptor[]) (PropertyFieldDescriptor[]) classprops
				.values().toArray(apropertyfielddescriptor);
	...
	public Object getPropertyField(String s) throws Exception {
		PropertyField propertyfield = getPropertyFieldDefine(s);
		if (propertyfield == null)
			throw new Exception("010108" + objclass.getName() + s);
		else
	...
	private HashMap getClassPropertys(Class class1) {
		...
		for (int i = 0; i < amethod.length; i++) {
			...
			for (int k = 0; k < amethod.length; k++) {
				...
				if (Modifier.isPublic(method4.getModifiers())
						&& method4.getName().equals(s))
					method1 = method4;
			}

			if (method1 != null) {
				hashmap.put(s2,
						new PropertyFieldDescriptor(s2, method, method1));
				continue;
			}
			boolean flag = false;
			int l = 0;
			do {
				...
				if (Modifier.isPublic(field1.getModifiers())
						&& field1.getName().equals(s2)) {
					flag = true;
					hashmap
							.put(s2,
									new PropertyFieldDescriptor(method, field1));
					break;
				}
				l++;
			} while (true);
		...
		for (int j = 0; j < afield.length; j++) {
			...
			String s1 = (new StringBuilder()).append("set").append(
					capitalize(s3)).toString();
			...
			for (int i1 = 0; i1 < amethod.length; i1++) {
				...
				if (Modifier.isPublic(method5.getModifiers())
						&& method5.getName().equals(s1))
					method2 = method5;
				if (method2 != null)
					hashmap
							.put(s3,
									new PropertyFieldDescriptor(field, method2));
				else
	...
	public static Object getProperty(Object obj, String s) throws Exception {
		if (obj == null) {
			return null;
		} else {
			PropertyFieldUtils propertyfieldutils = new PropertyFieldUtils(obj);
	...
	public static Object assign(Object obj, Object obj1) {
		...
		PropertyFieldDescriptor apropertyfielddescriptor[] = propertyfieldutils
				.getFieldDescriptor();
		for (int i = 0; i < apropertyfielddescriptor.length; i++) {
			PropertyFieldDescriptor propertyfielddescriptor = (PropertyFieldDescriptor) propertyfieldutils1
					.getClassPropMap().get(
							apropertyfielddescriptor[i].getPropertyName());
			if (propertyfielddescriptor != null)
				propertyfielddescriptor.set(obj1, apropertyfielddescriptor[i]
						.get(obj));
		}
